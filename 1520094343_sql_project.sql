...
JOIN Bookings ON Members.memid=Bookings.memid
JOIN Facilities ON Facilities.facid=Bookings.facid
WHERE starttime >= '2010-09-14' AND starttime < '2012-09-15'
HAVING cost>30
ORDER BY cost DESC;

/* Q9: This time, produce the same result as in Q8, but using a subquery. */
SELECT member, facility, cost
FROM (SELECT CONCAT (Members.surname, ',', Members.firstname) AS member, Facilities.name AS facility,
      CASE 
      ...
      WHERE Bookings.starttime >= '2010-09-14' AND Bookings.starttime < '2012-09-15') AS Bookings
      WHERE cost > 30
      ORDER BY cost DESC;
...
THEN Bookings.slots*Facilities.guestcost
ELSE (SUM(Bookings.slots)-Bookings.slots)*Facilities.guestcost + Bookings.slots*Facilities.membercost
END AS revenue
FROM Bookings
